...
public class ExposicionPerros
{
    ...
    public void ordenarPorRaza( )
    {
    	...
    	for (int i = perros.size(); i > 0; i--)
    	{
    		for (int j = 0; j < i - 1; j++)
    		{
    			Perro perro_uno = (Perro) perros.get(j);
    			Perro perro_dos = (Perro) perros.get(j+1);
    			if (perro_uno.compararPorRaza(perro_dos) > 0)
    			// si es > 0 significa que el perro_dos es menor que perro_uno.
    			{
    				perros.set(j, perro_dos);
    				perros.set(j+1, perro_uno);
    			}
    		}
    	}
    	verificarInvariante();
    ...
    public void ordenarPorNombre( )
    {
    	...
    	for (int i = 1; i < perros.size(); i++)
    	{
    		for (int j = i; j > 0; j--)
    		{
    			Perro perro_uno = (Perro) perros.get(j);
    			Perro perro_dos = (Perro) perros.get(j-1);
    			if (perro_uno.darNombre().compareTo(perro_dos.darNombre()) < 0)
    			// Significa que perro_uno > perro_dos alfabeticamente
    			{
    				perros.set(j,  perro_dos);
    				perros.set(j-1, perro_uno);
    			}
    ...
    public void ordenarPorPuntos( )
    {
    	...
    	for (int i = perros.size(); i > 0; i--)
    	{
    		for (int j = 0; j < i - 1; j++)
    		{
    			Perro perro_uno = (Perro) perros.get(j);
    			Perro perro_dos = (Perro) perros.get(j+1);
    			if (perro_uno.darPuntos() > perro_dos.darPuntos())
    			// si es > 0 significa que el perro_dos es menor que perro_uno.
    			{
    				perros.set(j, perro_dos);
    				perros.set(j+1, perro_uno);
    ...
    public void ordenarPorEdad( )
    {
    	...
    	{
    		Perro perro_menor = (Perro) perros.get(i);
    		int pos_Menor = i;
    		for (int j = i+1; j < perros.size(); j++)
    		{
    			Perro perro_siguiente = (Perro) perros.get(j);
    			if (perro_menor.darEdad() > perro_siguiente.darEdad())
    			{
    				perro_menor = perro_siguiente;
    				pos_Menor = j;
    			}
    		}
    		temporal = (Perro) perros.get(i);
    		perros.set(i, perro_menor);
    		perros.set(pos_Menor, temporal);
    	}
    	verificarInvariante();
    }
